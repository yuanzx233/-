...
export async function POST(request: NextRequest, context: Context) {
  try {
    ...
    const plans = generatePlanCandidates(JSON.parse(requirement.dataJson) as RequirementSubmission); const sequence = (await db.prepare("SELECT COALESCE(MAX(sequence), 0) + 1 AS sequence FROM project_versions WHERE project_id = ?").bind(id).first<{ sequence: number }>())?.sequence ?? 1; const versionId = crypto.randomUUID(); const now = new Date().toISOString();
