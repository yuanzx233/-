...
export function PlanWorkspace({ projectId, authHeaders, onConfirmed }: { projectId: string; authHeaders: Record<string, string>; onConfirmed: () => void }) {
  ...
  return <section className="plan-workspace" id="plans">
    <div className="cad-heading"><div><p className="eyebrow">DAY 5 · 平面方案生成</p><h2>先满足场地硬约束，再从 16 个模板中选择 3 套</h2></div><button className="submit-button plan-generate" type="button" disabled={loading} onClick={() => void generate()}>{loading ? "正在生成…" : plans.length ? "重新生成方案" : "生成候选方案"}</button></div>
    {error && <p className="error-banner">{error}</p>}
    {!plans.length && <div className="plan-empty"><strong>需求已保存，可以开始匹配</strong><p>先校验地块或控制线包含、首层占地率和保留对象避让，再对合格模板进行需求评分。</p></div>}
