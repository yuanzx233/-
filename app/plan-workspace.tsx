...
export function PlanWorkspace({ projectId, authHeaders, onConfirmed }: { projectId: string; authHeaders: Record<string, string>; onConfirmed: () => void }) {
  ...
  return <section className="plan-workspace" id="plans">
    <div className="cad-heading"><div><p className="eyebrow">DAY 5 · 平面方案生成</p><h2>从已审核 DXF 户型库匹配成熟方案</h2></div><button className="submit-button plan-generate" type="button" disabled={loading} onClick={() => void generate()}>{loading ? "正在生成…" : plans.length ? "重新匹配方案" : "生成候选方案"}</button></div>
