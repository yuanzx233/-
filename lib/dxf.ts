...
function renderBoundarySvg(boundary: SiteBoundary, analysis: DxfModel["siteAnalysis"], lines: Line2D[], polylines: Polyline2D[], circles: Circle2D[], texts: Text2D[]): string {
  ...
  const contourElevationValues = texts.filter((item) => item.layer.toUpperCase() === "CONTOUR").map((item) => Number.parseFloat(item.text.match(/(-?\d+(?:\.\d+)?)\s*m/i)?.[1] ?? "")).filter(Number.isFinite);
  const isSlopedTerrain = contourPolylines.length >= 2 && contourElevationValues.length >= 2 && Math.max(...contourElevationValues) - Math.min(...contourElevationValues) >= .5;
  ...
  const boundaryShape = `<polygon points="${points(boundary.points)}" fill="${isSlopedTerrain ? "#bfe0b8" : "#d8e4d2"}" fill-opacity=".88" stroke="#153b32" stroke-width="${round(stroke * 1.35)}"/>`;
  ...
  const terrainBands = isSlopedTerrain ? sortedContours.slice(0, -1).map((lower, index) => {
  ...
  const topBand = isSlopedTerrain && sortedContours.length ? `<polygon points="${points([...sortedContours.at(-1)!.points, { x: boundaryBox.maxX, y: boundaryBox.maxY }, { x: boundaryBox.minX, y: boundaryBox.maxY }])}" fill="${terrainPalette.at(-1)}" fill-opacity=".76" stroke="none" data-terrain="elevation-band" data-band-index="${sortedContours.length}"/>` : "";
  const terrainShapes = terrainBands + topBand + contourPolylines.map((item, index) => `<polyline points="${points(item.points)}" fill="none" stroke="#77674e" stroke-width="${round(stroke * .72)}" data-terrain="contour" data-contour-index="${index + 1}"/>`).join("")
    + elevationTexts.map((item) => { const point = map(item.position); const value = item.text.match(/(-?\d+(?:\.\d+)?)\s*m/i)?.[1] ?? "?"; const radius = stroke * 2.25; const triangle = `${round(point.x - radius * .866)},${round(point.y - radius * .5)} ${round(point.x + radius * .866)},${round(point.y - radius * .5)} ${round(point.x)},${round(point.y + radius)}`; return `<g data-terrain="elevation-point"><polygon points="${triangle}" fill="#8b4f31" data-elevation-symbol="triangle" data-tip-direction="down"/><text x="${point.x}" y="${round(point.y - radius * .5 - font * .28)}" text-anchor="middle" dominant-baseline="auto" fill="#613923" stroke="#f5f1e8" stroke-width="${round(stroke * .7)}" paint-order="stroke" font-size="${round(font * .55)}" font-family="Arial, sans-serif">${value} m</text></g>`; }).join("");
