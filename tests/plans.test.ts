...
test("loads all six approved mature templates", () => { assert.deepEqual(planTemplates.map(x => x.id), ["HT-T001", "HT-T002", "HT-T003", "HT-T004", "HT-T005", "HT-T006"]); assert.ok(planTemplates.every(x => x.status === "APPROVED")); });
...
test("returns candidates rendered from mature template geometry", () => { const plans = generatePlanCandidates(input as never); assert.equal(plans.length, 3); for (const plan of plans) { assert.match(plan.templateId, /^HT-T00[1-6]$/); assert.match(plan.templateSource, /STANDARDIZED_AC1032\.dxf/); assert.match(plan.svg, /成熟DXF户型及总尺寸/); assert.match(plan.svg, /data-layer="OVERALL_DIMENSIONS"/); assert.doesNotMatch(plan.svg, /data-layer="AXIS_GRID"/); assert.doesNotMatch(plan.svg, /data-layer="GRID_DIMENSIONS"/); assert.match(plan.svg, /总宽/); assert.match(plan.svg, /总深/); assert.ok(plan.rooms.length >= 6); assert.equal(plan.siteFit.fits, true); } });
