...
test("returns candidates rendered from mature template geometry", () => { const plans = generatePlanCandidates(input as never); assert.equal(plans.length, 3); for (const plan of plans) { assert.match(plan.templateId, /^HT-T00[1-6]$/); assert.match(plan.templateSource, /STANDARDIZED_AC1032\.dxf/); assert.match(plan.svg, /成熟DXF户型及轴网尺寸/); assert.match(plan.svg, /data-layer="AXIS_GRID"/); assert.match(plan.svg, /data-layer="GRID_DIMENSIONS"/); assert.match(plan.svg, /总宽/); assert.match(plan.svg, /总深/); assert.ok(plan.rooms.length >= 6); assert.equal(plan.siteFit.fits, true); } });
...
test("centers every room label and marks the main entrance", () => { const plans = generatePlanCandidates(input as never); for (const plan of plans) { assert.equal((plan.svg.match(/data-label-position="visual-center"/g) ?? []).length, plan.rooms.length); assert.match(plan.svg, /data-layer="MAIN_ENTRANCE"/); assert.match(plan.svg, />主入口<\/text>/); } });
