...
SRC = TD / "source/HT_T013_V0.1_SOURCE.dxf"
OUT = TD / "preview/HT_T013_V0.1_TWO_FLOOR_REVIEW_BASE.png"
...
panels = [("一层平面图", (20500, -1000, 40500, 10500)), ("二层平面图", (74600, -1000, 94600, 10500))]
W, H = 1800, 900
...
d.text((45, 25), "HT-T013 二层三房｜一层、二层独立复核底图", font=title, fill=(20, 35, 45))
d.text((48, 66), "深蓝=墙体｜青色=门窗｜紫色=楼梯｜灰色=轴线；请分别复核外墙、空间边界与楼梯对应关系", font=small, fill=(70, 80, 88))
for idx, (name, (minx, miny, maxx, maxy)) in enumerate(panels):
    x0, y0, pw, ph = 45 + idx * 875, 115, 830, 690
    s = min((pw - 50) / (maxx - minx), (ph - 70) / (maxy - miny))
    pt = lambda p: (x0 + 25 + (p[0] - minx) * s, y0 + ph - 35 - (p[1] - miny) * s)
    d.rounded_rectangle((x0, y0, x0 + pw, y0 + ph), 12, fill=(250, 252, 253), outline=(95, 115, 125), width=2)
    d.text((x0 + 22, y0 + 15), name, font=font, fill=(20, 45, 60))
    for ln in lines:
        sx, sy = ln["start"]; ex, ey = ln["end"]
        if max(sx, ex) < minx or min(sx, ex) > maxx or max(sy, ey) < miny or min(sy, ey) > maxy:
            continue
        layer, color, width = ln["layer"], (20, 48, 65, 255), 3
    ...
    for kind, data in ents:
        if kind != "INSERT" or base.first(data, 8, "") != "WINDOW":
            continue
